...
(function($){
	$.fn.j_scroll = function(options){
		...
		var singleSize = opt.type == "x" ? $this.children().outerWidth(true) : $this.children().outerHeight(); //获得单个子节点尺寸
		var overPlus = opt.type == "x" ? $this.outerWidth(true) - ChildSize* singleSize : $this.height() - ChildSize* singleSize;
			...
			if(opt.nextId){
				$("#" + opt.nextId).click(function(){
					...
					if(opt.action == "single"){
						...
						if(opt.type == "x"){
							doanimate(0-$this.outerWidth(true)/singleSize);
						}else{
			...
			if(opt.prevId){
				$("#" + opt.prevId).click(function(){
					...
					if(opt.action == "single"){
						...
						if(opt.type == "x"){
							doanimate($this.outerWidth(true)/singleSize);
						}else{
