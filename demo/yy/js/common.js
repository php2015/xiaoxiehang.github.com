...
$('a.menu-list-title').each(function(){
    ...
    self.click(function(e){
        ...
        self.parents('.main-menu').find('.icon-set').addClass('hide');
        if (self.parent('.menu-hd').find('.icon-set').length) {
            self.parent('.menu-hd').find('.icon-set').removeClass('hide');
        }
    })
})
...
function RetrunTag(tag,isDefault){
    ...
    if(isDefault){
        ...
        for(var i=0;i<tag.length;i++){
            htmlTag+='<li><a name="'+tag[i]['name']+'" target="_blank" data-newpage="'+tag[i]['newpage']+'" href="'+tag[i]['url']+'" title="'+tag[i]['title']+'" ><i class="icon icon-menu-item"></i>'+tag[i]['title']+'</a></li>';
        }
    }
    return htmlTag;
}
...
function menuDefault(){
    var m='',n='';
    for(var i=0;i<manage.sales.length;i++){
        m+='<li><a name="'+manage.sales[i]['name']+'" target="_blank" data-newpage="'+manage.sales[i]['newpage']+'" href="'+manage.sales[i]['url']+'" title="'+manage.sales[i]['title']+'" ><i class="icon icon-menu-item"></i>'+manage.sales[i]['title']+'</a></li>';
